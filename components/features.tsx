...
export function Features() {
  return (
    <div className="flex flex-col items-center justify-center lg:container mt-[90px] mb-10">
      <div
        className={cn(
      ...
      >
        {data.map((item, index) => (
          <MagicCard
            key={index}
            className="flex w-full sm:w-[400px] sm:h-[300px] lg:w-[48%] xl:w-[32%] cursor-pointer flex-col items-center justify-center overflow-hidden bg-[radial-gradient(var(--mask-size)_circle_at_var(--mouse-x)_var(--mouse-y),#ffaa40_0,#9c40ff_50%,transparent_100%)] p-8 shadow-2xl"
          >
            {item.image}

            <h1 className="z-10 whitespace-nowrap text-2xl font-medium text-gray-800 dark:text-gray-200 mt-3">
              {item.title}
            </h1>
            <p className="text-center text-sm mt-3">{item.description}</p>
